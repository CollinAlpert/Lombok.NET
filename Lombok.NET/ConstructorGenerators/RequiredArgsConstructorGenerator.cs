...
public class RequiredArgsConstructorGenerator : BaseConstructorGenerator
{
	...
	protected override (SyntaxKind modifier, ParameterListSyntax constructorParameters, BlockSyntax constructorBody) GetConstructorParts(TypeDeclarationSyntax typeDeclaration, AttributeData attribute)
	{
		...
		var modifierType = (ModifierType?)(modifierTypeArgument.Value.Value as int?) ?? ModifierType.WithClass;
		var modifier = modifierType switch
		{
			ModifierType.Public => SyntaxKind.PublicKeyword,
			ModifierType.Internal => SyntaxKind.InternalKeyword,
			ModifierType.Protected => SyntaxKind.ProtectedKeyword,
			ModifierType.Private => SyntaxKind.PrivateKeyword,
			_ or ModifierType.WithClass => typeDeclaration.GetAccessibilityModifier(),
		};
		switch (memberType)
		{
